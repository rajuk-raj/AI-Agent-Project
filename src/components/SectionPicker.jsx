 ...
 */
export default function SectionPicker({ analysis, worked = {}, onPick, onPickCustom }) {
  ...
  const resume = all.filter((s) => s.source === 'resume');
  const notes = all.filter((s) => s.source === 'experience');

  const Group = ({ title, sections, hint }) =>
  ...
  return (
    <div className="space-y-6">
      ...
      <Group
        ...
        sections={resume}
        hint="Points already on your resume — rewritten in place."
      />
      <Group
        title="From your experience notes"
        sections={notes}
        hint="Work you’ve written down but not yet put on your resume."
      />
